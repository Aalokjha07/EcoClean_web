...
// Distinct blue circle icon for current user position
...
// Helper component to update map view and draw routes dynamically
const RoutingAndController = ({
  destination,
  userLocation,
  baseCoords,
  setRoutingControl,
  ...
  useEffect(() => {
    ...
    // Pan map to chosen task
    map.setView([destination.lat, destination.lng], 15);

    const origin = userLocation || baseCoords;

    // Remove any previously existing route control
    ...
    const control = L.Routing.control({
      ...
      show: false, // Hides written turn-by-turn panel
    }).addTo(map);

    map._routingControl = control;
    setRoutingControl(control);
  }, [destination, map, userLocation, baseCoords]);
...
const FleetMap = () => {
  ...
  useEffect(() => {
    ...
    // Fetch user's live position on startup
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          setUserLocation({
        ...
        (error) => {
          console.warn(
            "Geolocation unavailable or denied. Using fallback.",
            error,
          );
          setUserLocation(baseCoords);
        },
      );
    } else {
      setUserLocation(baseCoords);
  ...
  const handleStartNavigation = () => {
    ...
    const originLat = userLocation?.lat || baseCoords.lat;
    const originLng = userLocation?.lng || baseCoords.lng;

    const url = `https://www.openstreetmap.org/directions?engine=fossgis_osrm_car&route=${originLat}%2C${originLng}%3B${taskCoords[0]}%2C${taskCoords[1]}`;
    window.open(url, "_blank");
  };
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-slate-900 overflow-hidden font-['Plus_Jakarta_Sans']">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* LEFT PART: MAP */}
        <div className="flex-[7] relative h-full bg-slate-200 z-0">
          {userLocation ? (
            <MapContainer
          ...
          <div className="absolute top-4 left-4 bg-white/90 backdrop-blur px-4 py-2 rounded-xl shadow-xl border border-white flex items-center gap-2 z-[400]">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-700">
              Live Satellite Feed (OSM)
            </span>
          </div>

          {selectedTask && (
            <div className="absolute bottom-6 left-6 right-6 bg-slate-900/95 backdrop-blur-md text-white p-4 rounded-3xl shadow-2xl border border-slate-700 flex items-center justify-between z-[400] animate-in slide-in-from-bottom-10 duration-300">
              <div className="flex items-center gap-4 border-r border-slate-700 pr-6 mr-6">
                <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg">
                  <Navigation className="text-white w-6 h-6" />
                </div>
